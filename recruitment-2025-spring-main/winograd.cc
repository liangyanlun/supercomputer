...
//#pragma omp parallel for
  ...
  float *B_flat=(float*)malloc(N*K * sizeof(float));
  float *A_flat=(float*)malloc(M*K * sizeof(float));
  float *C_flat=(float*)malloc(N*M * sizeof(float));
  #pragma omp parallel for schedule(static)
for(int64_t i = 0; i < M; i++)
{
  for(int64_t k = 0;k < K; k++)
  {
    A_flat[i * K + k] = A_tensor[i][k];
  }
}
#pragma omp parallel for schedule(static)
...
for(int64_t i = 0; i < N; i++)
{
  ...
  #pragma omp parallel for schedule(static)  // 静态调度，避免 false sharing
for (int64_t m = 0; m < M; ++m) {
    // 每个线程处理一行（m），内部完全独立
    for (int64_t n = 0; n + 7 < N; n += 8) {
        // 初始化 8 个 SIMD 累加器
        ...
        for (int64_t k = 0; k < K; ++k) {
            // 广播 A_flat[m][k] 到向量
            __m256 a = _mm256_broadcast_ss(&A_flat[m * K + k]);
            // 加载 B_flat[k][n: n+7]
            __m256 b = _mm256_load_ps(&B_flat[k * N + n]);
        ...
        // 存储结果到 C[n+i][m]
        _mm256_storeu_ps(&C_flat[m * N + n], c);
    }

    // 处理尾部元素（当 N 不是 8 的倍数时）
    for (int64_t n = (N / 8) * 8; n < N; ++n) {
        float sum = 0.0f;
        for (int64_t k = 0; k < K; ++k) {
            sum += A_flat[m * K + k] * B_flat[k * N + n];
        }
        C_flat[m * N + n] = sum;
    }
}
